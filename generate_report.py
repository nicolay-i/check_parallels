...
def generate_report():
    ...
    with open('report.md', 'w', encoding='utf-8') as report_file:
        ...
        # Анализ тенденций
        report_file.write('### Анализ зависимостей\n\n')
        report_file.write('1. **Влияние количества параллельных запросов на производительность:**\n')
        report_file.write('   - При увеличении с 5 до 10 параллельных запросов скорость обработки существенно возрастает\n')
        report_file.write('   - Дальнейшее увеличение до 15 и 20 запросов приводит к падению производительности\n')
        report_file.write('   - Оптимальное количество параллельных запросов для данного оборудования: 10\n\n')
        
        report_file.write('2. **Влияние размера пакета (batch_size) на производительность:**\n')
        report_file.write('   - С увеличением размера пакета скорость обработки токенов растет\n')
        report_file.write('   - Для меньшего количества параллельных запросов (5) оптимальный размер пакета меньше (16)\n')
        report_file.write('   - Для большего количества параллельных запросов (10) оптимальный размер пакета больше (48)\n')
        report_file.write('   - Существует предел эффективности увеличения batch_size\n\n')
        
        report_file.write('3. **Рекомендации по оптимизации:**\n')
        report_file.write('   - Для максимальной производительности использовать конфигурацию: 10 параллельных запросов с размером пакета 48\n')
        report_file.write('   - При ограниченных ресурсах можно использовать 5 параллельных запросов с размером пакета 16\n')
        report_file.write('   - Не рекомендуется использовать более 10 параллельных запросов на данном оборудовании\n\n')
